...
public class Network {
    ...
    private double[][][] weights; // 3D: 1st for layer, 2nd for neuron, 3rd for neuron in previous
                                  // layer.(weights[layer][neuron][prevNeuron])
    ...
    public Network(int... NETWORK_LAYER_SIZES) // ... : 0 or more arguments may be passed through. this.* =
                                               // NETWORK_LAYER_SIZES sets each next value placed in function paramater
    ...
    { // (makes an array out of NETWORK_LAYER-SIZES inputs)
        ...
        for (int i = 0; i < NETWORK_SIZE; i++) {
            this.output[i] = new double[NETWORK_LAYER_SIZES[i]]; // each neuron has 1 output, 1 error signal, and 1
                                                                 // output derivative.
            this.error_signal[i] = new double[NETWORK_LAYER_SIZES[i]];
            this.output_derivative[i] = new double[NETWORK_LAYER_SIZES[i]];

            this.bias[i] = NetworkTools.createRandomArray(NETWORK_LAYER_SIZES[i], -0.5, 0.7); // may need to change
            ...
            {
                weights[i] = NetworkTools.createRandomArray(NETWORK_LAYER_SIZES[i], NETWORK_LAYER_SIZES[i - 1], -1.0,
                        1.0); // .. if learning ever gets stuck at a certain high error number. It increases
                              // initial scope so it will learn more of the learning curve.
            }
        }
    }
    ...
    public double[] calculate(double... input) {
        ...
        for (int layer = 1; layer < NETWORK_SIZE; layer++) // loop through each layer
        {
            ...
            {
                double sum = bias[layer][neuron]; // setting sum to the bias to start for part1 (addition in formula;
                                                  // intercept)
                for (int prevNeuron = 0; prevNeuron < NETWORK_LAYER_SIZES[layer - 1]; prevNeuron++) // loop through each
                                                                                                    // previous neuron
                {
                    sum += output[layer - 1][prevNeuron] * weights[layer][neuron][prevNeuron]; // Completes part1 of
                                                                                               // formula
                                                                                               // (multiplication in
                                                                                               // formula)
                }
        ...
        return output[NETWORK_SIZE - 1]; // returns output. -1 because index start at 0 and last index is the
                                         // arraylength (NETWORK_SIZE) -1.
    }
    ...
    public double MSE(double[] input, double[] target) // Mean square error. Calculates how close output and target
                                                       // vectors are
    {
    ...
    public double MSE(TrainSet set) // Mean square error. Calculates the MSE for each of the datasets and returns
                                    // the average
    {
        double v = 0;
    ...
    public void backpropError(double[] target) // calculates error using the error formula
    {
        for (int neuron = 0; neuron < NETWORK_LAYER_SIZES[NETWORK_SIZE - 1]; neuron++) {
            error_signal[NETWORK_SIZE - 1][neuron] = (output[NETWORK_SIZE - 1][neuron] - target[neuron])
                    * output_derivative[NETWORK_SIZE - 1][neuron]; // finds the error signal in a neuron in current
                                                                   // layer
        }
        for (int layer = NETWORK_SIZE - 2; layer > 0; layer--) // NETWORK_SIZE-2 is index of last hidden layer. layer >
                                                               // 0 because we ignore input layer as it just passes data
                                                               // through and does not have error.
        {
            for (int neuron = 0; neuron < NETWORK_LAYER_SIZES[layer]; neuron++) // go through every neuron in layer
            {
                double sum = 0;
                for (int nextNeuron = 0; nextNeuron < NETWORK_LAYER_SIZES[layer + 1]; nextNeuron++) // layer+1 because
                                                                                                    // that is the layer
                                                                                                    // that the next
                                                                                                    // neuron is in
                {
                    sum += weights[layer + 1][nextNeuron][neuron] * error_signal[layer + 1][nextNeuron]; // increase sum
                                                                                                         // by weight
                                                                                                         // that
                                                                                                         // connects
                                                                                                         // next neuron
    ...
    public void updateWeights(double eta) {
        for (int layer = 1; layer < NETWORK_SIZE; layer++) // starts at first hidden layer, until reach output layer
        {
            ...
            {
                // each neuron has multiple weights (from having multiple connections), but only
                // one bias, so it is done outside of weights loop.
                double delta = -eta * error_signal[layer][neuron]; // delta is change in bias
                bias[layer][neuron] += delta;
                for (int prevNeuron = 0; prevNeuron < NETWORK_LAYER_SIZES[layer - 1]; prevNeuron++) // go through each
                                                                                                    // previous neuron
                                                                                                    // to get each
                                                                                                    // weight
                {
                    weights[layer][neuron][prevNeuron] += delta * output[layer - 1][prevNeuron]; // Uses previous delta
                                                                                                 // because it is less
                                                                                                 // calculations to do
                                                                                                 // and is the same
                                                                                                 // formula.
    ...
    public static void main(String[] args)
    {
        ...
        try {
        //    Network net = loadNetwork("res/test2.txt");
        } catch (Exception e) {
            // Auto-generated catch block
            e.printStackTrace();
        }
        // generally have the highest amount of neurons first, and go down until the
        // lowest (i.e no 4,100,3,2)
        // Network net = new Network(4, 3, 2); // first[0]: amount of input neurons. In between[1->last]: amount of neurons
                                            // in each hidden layer. last[length-1]: amount of output neurons.
        // MATCH input and output data amounts with input and output neuron amounts,
        // else data won't match Network and it will not train.
        // try { // try and catch handle exceptions
        //     Network net = Network.loadNetwork("res/test.txt");
